...
def patient_record_view(request):
    ...
    try:
        if form.is_valid():
            ...
            form.add_error('full_name', 'Please enter a valid name format.')
            
    return render(request, 'myapp/patient_record_view.html', {'form' : form, 'patient_records' : patient_records})
...
def appointment_schedule_view(request):
    ...
    if sort_by == 'view':
        #Sorts by Date
        appointment_schedules = Appointment_Schedule.objects.filter(current_appointment_schedule__gte=current_date).order_by('current_appointment_schedule', 'appointment_time')
    elif sort_by == 'name':
        #Sorts by Name
        appointment_schedules = Appointment_Schedule.objects.filter(current_appointment_schedule__gte=current_date).order_by('patient_id_id__last_name', 'patient_id_id__first_name' , 'current_appointment_schedule')
    else:
        #This first one gets displayed is appointment_schedules for TODAY ONLY, AS IN THE DATE TODAY
        appointment_schedules = Appointment_Schedule.objects.filter(current_appointment_schedule__range=(current_date, current_date))
 
    #search by date
    form1 = Search_Date(request.GET)

    if form1.is_valid():
        date1 = form1.cleaned_data['date']
        appointment_schedules = Appointment_Schedule.objects.filter(current_appointment_schedule=date1)
    ...
    context = {'appointment_schedules': appointment_schedules, 'form1' : form1, 'form' : form}

    appointment_schedules = Appointment_Schedule.objects.all()
    return render(request, 'myapp/appointment_schedule_view.html', context)
...
def appointment_schedule_view_present(request):
    ...
    if form1.is_valid():
        date1 = form1.cleaned_data['date']
        appointments = Appointment_Schedule.objects.filter(current_appointment_schedule=date1)
    ...
    try:
        if form.is_valid():
            ...
            last_name = form.cleaned_data['last_name']
            patient_records = Patient_Records.objects.filter(last_name__icontains=last_name, first_name__icontains=first_name)
            appointments = Appointment_Schedule.objects.filter(patient_id__in=patient_records)
    except ValueError:
            form.add_error('full_name', 'Please enter a valid name format.')

    context = {'appointments': appointments, 'form1' : form1, 'form' : form}
